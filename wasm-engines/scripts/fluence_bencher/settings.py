...
vm_descriptors = {
    ...
    "wabt"   : VMDescriptor("wabt", "{wasm_file_path} --run-all-exports"),
    "vanilla-wabt" : VMDescriptor("vanilla-wabt", "{wasm_file_path} --run-all-exports"),
    "v8-liftoff" : VMDescriptor("node", "--liftoff --no-wasm-tier-up scripts/node-timer.js {wasm_file_path}"),
    "v8-turbofan" : VMDescriptor("node", "--no-liftoff scripts/node-timer.js {wasm_file_path}"),
    ...
    "wamr-aot" : VMDescriptor("wamr_aot.sh", "{function_name} {wasm_file_path}", script_invoke=True), 
    "wasm3" : VMDescriptor("wasm3", "--func {function_name} {wasm_file_path}"),
    "fizzy" : VMDescriptor("fizzy.sh", "{function_name} {wasm_file_path}", script_invoke=True),
