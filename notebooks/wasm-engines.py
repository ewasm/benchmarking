...
# a graph of bls12 pairing: evm384-v7 estimated, wasm (wabt-with-bignums), rust native
...
fig_plt = plotOneTestUsingTimeCol(df_evm384_versions_summarized,
            suptitle="bls12-pairing: evm384 (estimated via adjustment factor) vs native and interpreted wasm",
            suptitle_pos=1.02)

fig_plt.savefig('../images/evm384-vs-native-wasm.png', bbox_inches='tight')

# a graph of all evm384 versions and languages

import pdb; pdb.set_trace()

df_evm384_all_versions = df_evm384_wasm_native.copy()
df_evm384_all_versions.reset_index(inplace=True)


df_evm384_all_versions = df_evm384_all_versions[df_evm384_all_versions['engine_bench_name'].str.contains(pat='evm384-synth-loop')]
df_evm384_all_versions.set_index('engine_bench_name', inplace=True)

fig_plt = plotOneTestUsingTimeCol(df_evm384_all_versions,
            suptitle="f6m_mul synth loop: evm384 versions compared",
            suptitle_pos=1.02)

fig_plt.savefig('../images/evm384-all-versions.png', bbox_inches='tight')
